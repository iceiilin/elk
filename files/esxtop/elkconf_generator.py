...
def create_kibana(heading_list, template, configure_file, timestamp):
    ...
    for configure in kibana_json_example:
        if configure["_type"] == "visualization":
            ...
            tmpconf["index"] = index_name
            configure["_source"]["kibanaSavedObjectMeta"]["searchSourceJSON"] = json.dumps(tmpconf)
            visualization_json_list.append(configure)
        elif configure["_type"] == "dashboard":
            configure["_source"]["title"] = index_name
            configure["_id"] = index_name
            tmpconf = json.loads(configure["_source"]["panelsJSON"])
            for (index, conf) in enumerate(tmpconf):
                conf["id"] = vis_titles[index]
            configure["_source"]["panelsJSON"] = json.dumps(tmpconf)
    ...
    for heading in heading_list:
        for (index, patten) in enumerate(vis_pattens):
            if patten.match(heading):
                ...
                # TODO: customize legend in Y axis
                # metric_copy["params"]["customLable"] = 
...
if __name__ == "__main__":
    # for debug purpose
    create_kibana(HEADINGS, "esxtop_kibana.template", "esxtop_debug.kibana", "2010_05")
