...
# parser.add_argument("--suffix", action="store", default="", help="suffix used index name")
...
kibana_files = glob.glob(EXECUTE_PATH+"/*.kibana")
index_list = []
print kibana_files
for fpath in kibana_files:
    index_list.append(os.path.splitext(os.path.basename(fpath))[0])

print index_list
# index_list = ["benchmark_" + suffix, "esxtop_" + suffix, "rackhdlog_" + suffix]
...
for kibana_configure in kibana_files:
    ...
    with open(kibana_configure, "r") as f:
